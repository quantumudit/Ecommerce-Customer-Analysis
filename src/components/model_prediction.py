...
from src.utils.basic_utils import load_pickle, read_yaml
...
class ModelPrediction:
    ...
    def predict(self, data: pd.DataFrame) -> float:
        ...
        Returns:
            float: _description_
        """
        preprocessor = load_pickle(self.preprocessor_path)
        model = load_pickle(self.model_path)

        normalized_data_array = preprocessor.transform(data)
        predicted_value = model.predict(normalized_data_array)
